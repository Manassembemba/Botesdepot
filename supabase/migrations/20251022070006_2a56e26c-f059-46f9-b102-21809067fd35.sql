...
-- Enum pour les rôles (avec vérification d'existence)
do $$ 
begin
...
-- =====================================================
create table if not exists public.locker_stock (
  id bigserial primary key,
...
drop trigger if exists trg_locker_stock_timestamp on public.locker_stock;
...
begin
  ...
  if current_qty is null then
    raise exception 'Aucun stock trouvé pour ce casier/produit';
  end if;
...
begin
  insert into public.sales(reference, created_by, total_amount, payment_status)
  values (p_reference, p_created_by, 0, 'pending')
...
drop policy if exists "Admin/magasinier can manage products" on public.products;
drop policy if exists "Everyone can read lockers" on public.lockers;
drop policy if exists "Admin/magasinier can manage lockers" on public.lockers;
drop policy if exists "Everyone can read stock" on public.locker_stock;
drop policy if exists "Admin/magasinier can manage stock" on public.locker_stock;
drop policy if exists "Everyone can read stock entries" on public.stock_entries;
...
drop policy if exists "Admin/magasinier can create sales" on public.sales;
...
drop policy if exists "Admin/magasinier can create sale items" on public.sale_items;
